...
class WreathEditorDialog(QDialog):
    ...
    def populate_fields(self):
        ...
        self.images_label.setText(f"Current images: {image_count} image(s)")
    ...
    def accept_changes(self):
        ...
        if 'id' not in self.wreath_data:
            self.wreath_data['id'] = str(uuid.uuid4())
        if 'dateAdded' not in self.wreath_data:
            self.wreath_data['dateAdded'] = datetime.now().isoformat()
        if 'images' not in self.wreath_data:
