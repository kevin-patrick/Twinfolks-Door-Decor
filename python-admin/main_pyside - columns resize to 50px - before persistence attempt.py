...
class WreathTableWidget(QTableWidget):
    ...
    def on_column_resized(self, logical_index, old_size, new_size):
        ...
        # Only save for draggable columns (1=Title, 4=Price, 5=Hashtags, 6=Description)
        draggable_columns = [1, 4, 5, 6]
        if logical_index in draggable_columns:
            # Ensure minimum width of 50px
            if new_size < 50:
                self.setColumnWidth(logical_index, 50)
                return
            
            # Save to parent's settings if this table has a parent
            if hasattr(self.parent(), 'save_column_width'):
                self.parent().save_column_width(logical_index, new_size)
