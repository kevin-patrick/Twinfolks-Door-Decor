...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        column_reset_layout = QHBoxLayout()
        column_reset_btn = QPushButton("Restore Default Column Widths")
        column_reset_btn.clicked.connect(self.restore_default_column_widths)
        column_reset_layout.addWidget(column_reset_btn)
        column_reset_layout.addStretch()
        layout.addLayout(column_reset_layout)
        
        # Buttons
        button_box = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        button_box.accepted.connect(self.debug_accept)
        button_box.rejected.connect(self.reject)
        layout.addWidget(button_box)

    def debug_accept(self):
        """Debug method to see what's happening when OK is clicked"""
        print("DEBUG: OK button was clicked!")
        
        # Check if fields exist
        print(f"Fields exist: site_id={hasattr(self, 'site_id_edit')}, token={hasattr(self, 'access_token_edit')}")
        
        if hasattr(self, 'site_id_edit'):
            site_id_text = self.site_id_edit.text()
            print(f"Site ID field contains: '{site_id_text}'")
        
        if hasattr(self, 'access_token_edit'):
            token_text = self.access_token_edit.text()
            print(f"Access Token field contains: '{token_text[:10] if token_text else ''}'...")
        
        # Now call the real accept method
    ...
    def accept_settings(self):
        ...
        # Debug what we're reading from the dialog fields
        site_id_from_field = self.site_id_edit.text().strip()
        token_from_field = self.access_token_edit.text().strip()
        
        print(f"SETTINGS DIALOG READING: site_id='{site_id_from_field}', token='{token_from_field[:8] if token_from_field else ''}'...")
        
        self.settings['netlify_site_id'] = site_id_from_field
        self.settings['netlify_access_token'] = token_from_field
        self.settings['auto_backup'] = self.auto_backup_cb.isChecked()
        self.settings['backup_count'] = self.backup_count_spin.value()
        # Check if the fields even exist
        print(f"FIELD EXISTS CHECK: site_id_edit exists={hasattr(self, 'site_id_edit')}, access_token_edit exists={hasattr(self, 'access_token_edit')}")
        if hasattr(self, 'site_id_edit'):
            print(f"SITE ID FIELD VALUE: '{self.site_id_edit.text()}'")
        if hasattr(self, 'access_token_edit'):
            print(f"ACCESS TOKEN FIELD VALUE: '{self.access_token_edit.text()}'")
        ...
        print(f"SETTINGS DIALOG SAVING: netlify_site_id='{self.settings.get('netlify_site_id')}', netlify_access_token='{self.settings.get('netlify_access_token')}'")
        self.accept()
    ...
    def restore_default_column_widths(self):
        ...
        if hasattr(self.parent(), 'restore_default_column_widths'):
            self.parent().restore_default_column_widths()
            QMessageBox.information(self, "Success", "Column widths restored to defaults!")